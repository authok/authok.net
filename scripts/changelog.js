if (process.platform === 'win32') {
  ...
  module.exports = function (newVersion) {
    return new Promise((resolve, reject) => {
      ...
      const webtask = `https://webtask.it.authok.cn/api/run/wt-hernan-authok_com-0/oss-changelog.js?webtask_no_cache=1&repo=${repo}&milestone=${newVersion}`;
      ...
      stream.once('open', function (fd) {
        ...
        stream.write(
          `## [release-${newVersion}](https://github.com/authok/${repo}/tree/release-${newVersion}) (${moment().format(
            'YYYY-MM-DD'
          )})`
        );
        ...
        stream.write(
          `[Full Changelog](https://github.com/authok/${repo}/compare/release-${currentVersion}...release-${newVersion})`
        );
