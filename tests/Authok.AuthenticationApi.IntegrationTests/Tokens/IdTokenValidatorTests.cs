...
namespace Authok.AuthenticationApi.IntegrationTests.Tokens
{
    public class IdTokenValidatorUnitTests : TestBase
    {
        ...
        static readonly IdTokenRequirements defaultReqs =
            new IdTokenRequirements(JwtSignatureAlgorithm.HS256, "https://authok-dotnet-integration-tests.authok.cn/", "tokens-test-123", TimeSpan.FromMinutes(1))
            {
                Nonce = "a1b2c3d4e5",
        ...
        {
            return idTokenValidator.Assert(defaultReqs, token, GetVariable("AUTHOK_CLIENT_SECRET"), tokensWereValid);
        }
