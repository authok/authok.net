...
namespace Authok.AuthenticationApi.IntegrationTests.Tokens
{
    public class IdTokenClaimValidatorTests : TestBase
    {
        ...
        static readonly IdTokenRequirements defaultReqs =
            new IdTokenRequirements(JwtSignatureAlgorithm.RS256, "https://tokens-test.authok.cn/", "tokens-test-123", TimeSpan.FromMinutes(1))
            {
                Nonce = "a1b2c3d4e5",
        ...
        public void ThrowsWhenIssInvalid()
        {
            ...
            var ex = Assert.Throws<IdTokenValidationException>(() => ValidateToken(token));
            Assert.Equal("Issuer (iss) claim mismatch in the ID token; expected \"https://tokens-test.authok.cn/\", found \"something-else\".", ex.Message);
        }
        ...
        public void SucceedsWhenNonceShouldBeIgnored()
        {
            ...
            var reqs = new IdTokenRequirements(JwtSignatureAlgorithm.RS256, "https://tokens-test.authok.cn/", "tokens-test-123", TimeSpan.FromMinutes(5))
            {
                MaxAge = TimeSpan.FromSeconds(100)
            };
