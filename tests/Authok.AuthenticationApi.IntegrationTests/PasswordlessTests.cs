...
namespace Authok.AuthenticationApi.IntegrationTests
{
    public class PasswordlessTests : TestBase
    {
        ...
        {
            authenticationApiClient = new AuthenticationApiClient(GetVariable("AUTHOK_AUTHENTICATION_API_URL"));
            email = GetVariable("AUTHOK_PASSWORDLESSDEMO_EMAIL", false);
            phone = GetVariable("AUTHOK_PASSWORDLESSDEMO_PHONE", false);
        }

        [SkippableFact]
        public async Task Can_launch_email_link_flow()
        {
            Skip.If(string.IsNullOrEmpty(email), "AUTHOK_PASSWORDLESSDEMO_EMAIL not set");
        ...
        public async Task Can_launch_email_link_flow_with_auth_parameters()
        {
            ...
            var request = new PasswordlessEmailRequest
            {
                ...
                AuthenticationParameters = new Dictionary<string, object>()
                {
                    ...
                    { "scope" , "openid" },
                    { "nonce" , "mynonce" },
                    { "redirect_uri", "http://localhost:5000/signin-authok" }
                }
            };
        ...
        {
            Skip.If(string.IsNullOrEmpty(phone), "AUTHOK_PASSWORDLESSDEMO_PHONE not set");

            // Arrange
            using (var authenticationApiClient = new AuthenticationApiClient(GetVariable("AUTHOK_PASSWORDLESSDEMO_AUTHENTICATION_API_URL")))
            {
                // Act
                var request = new PasswordlessSmsRequest
                {
                    ClientId = GetVariable("AUTHOK_PASSWORDLESSDEMO_CLIENT_ID"),
                    ClientSecret = GetVariable("AUTHOK_PASSWORDLESSDEMO_CLIENT_SECRET"),
                    PhoneNumber = phone
                };
        ...
        public async Task Should_Request_a_token_using_sms_code()
        {
            ...
            var domain = GetVariable("AUTHOK_AUTHENTICATION_API_URL");
            ...
            var expectedParams = new Dictionary<string, string>
            {
                ...
                { "otp", code },
                { "client_id", GetVariable("AUTHOK_CLIENT_ID") },
                { "client_secret", GetVariable("AUTHOK_CLIENT_SECRET") },
                { "audience", GetVariable("AUTHOK_MANAGEMENT_API_AUDIENCE") },
                { "username", "0123456789" },
                { "scope", "openid email" }
            };
