...
namespace Authok.AuthenticationApi.Tokens
{
    ...
    internal static class IdTokenClaimValidator
    {
        ...
        public static void AssertClaimsMeetRequirements(IdTokenRequirements required, JwtSecurityToken token, DateTime? pointInTime = null)
        {
            ...
            {
                var organization = GetClaimValue(token.Claims, AuthokClaimNames.Organization);
        ...
        /// </summary>
        /// <param name="claims"><see cref="IEnumerable{Claim}"/>Claims to search the <paramref name="claimType"/> for.</param>
        /// <param name="claimType">Type of claim to search the <paramref name="claims"/> for. See <see cref="JwtRegisteredClaimNames"/> or <see cref="AuthokClaimNames"/> for possible names.</param>
